"""Add a Fusion-style revolute joint between a Brep circular edge and a fixed axis."""
...
def RunCommand(is_interactive):
    try:
        edge = prompt_for_circular_brep_edge(
            "Select circular/arc edge(s) on body for fixed-axis revolute joint, then Enter",
            role="revolute_body",
        )
        axis = pick_axis_by_two_points(
            "Pick fixed revolute axis start point",
            "Pick fixed revolute axis end point",
            role="fixed_revolute_axis",
            source="two_points",
        )
        joint_records, session_module, generate_definition = _reload_assembly_modules()
        record = joint_records.revolute_to_axis_joint_record(body_edge=edge, axis=axis)
        ...
        print("Added fixed-axis revolute joint {}".format(record["id"][:8]))
        print(session_module.session_summary(session))
        return Result.Success
    except (PromptCancelled, EdgePromptCancelled) as error:
        print("AssemblyGH fixed-axis revolute joint cancelled: {}".format(error))
        return Result.Cancel
    except EdgeReferenceError as error:
        print("AssemblyGH fixed-axis revolute joint invalid selection: {}".format(error))
